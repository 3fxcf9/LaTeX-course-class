\documentclass{course}
\usepackage[utf8]{inputenc}
\usepackage{marginnote}

% Code listings
\usepackage{listings}
\usepackage{sourcecodepro}
\lstset{language=tex}
\lstset{
    basicstyle=\ttfamily\footnotesize\fontfamily{SourceCodePro-TLF}\selectfont,
    breaklines=true,
    keepspaces=true
}

\theoremstyle{unnumbered}
\newtheorem{code}{Code}
\tcolorboxenvironment{code}{
	boxrule=0pt,
	boxsep=0pt,
	blanker,
	borderline west={1pt}{0pt}{black},
	sharp corners,
	before skip=8pt,
	after skip=8pt,
	left=5pt,
	right=5pt,
	breakable,
}


\title{Course class documentation}
\subtitle{A \LaTeX\hspace{0.1em} class for writing courses}
\author{Felix RONDEAU}


\begin{document}
\maketitle
\begin{lstlisting}
\title{course.cls}
\subtitle{A \LaTeX class for writing courses}
\author{Felix RONDEAU}
\maketitle
\end{lstlisting}

\section{Description}
\subsection{Class features}
\subsubsection{Provided environments}
\begin{lstlisting}
\section{Description}
\subsection{Class features}
\subsubsection{Provided environments}
\end{lstlisting}

\begin{definition}
	\begin{lstlisting}
\begin{definition}
	...
\end{definition}
\end{lstlisting}
\end{definition}

\begin{theorem}
	\begin{lstlisting}
\begin{theorem}
	...
\end{theorem}
\end{lstlisting}
\end{theorem}

\begin{proof}
	\begin{lstlisting}
\begin{proof}
	...
\end{proof}
\end{lstlisting}
	Ended by a QED $\rightarrow$
\end{proof}

\begin{lemma}
	\begin{lstlisting}
\begin{lemma}
	...
\end{lemma}
\end{lstlisting}
\end{lemma}

\begin{corollary}
	\begin{lstlisting}
\begin{corollary}
	...
\end{corollary}
\end{lstlisting}
\end{corollary}

\begin{example}
	\begin{lstlisting}
\begin{example}
	...
\end{example}
\end{lstlisting}
\end{example}

\begin{remark}
	\begin{lstlisting}
\begin{remark}
	...
\end{remark}
\end{lstlisting}
\end{remark}


\subsubsection{Aliases}
The class provides aliases for commonly used mathematical objects. Here are some examples.\\
Mathematical sets
\[
	\P \subset \N \subset \Z \subset \Q \subset \R \subset \C
\]
\begin{code}
	\begin{lstlisting}
\P \subset \N \subset \Z \subset \Q \subset \R \subset \C
\end{lstlisting}
\end{code}

\vspace{1em}\noindent Complex numbers and vectors
\[
	\Re\Bigl(\conj{(b-a)} \times (d-c)\Bigr) = \vec{AB} \cdot \vec{CD} = \norm{\vec{AB}} \times \norm{\vec{CD}} \times \cos\Bigl(\widehat{\vec{AB},\vec{CD}}\Bigr)
\]

\begin{code}
	\begin{lstlisting}
\Re\Bigl(\conj{(b-a)} \times (d-c)\Bigr) = \vec{AB} \cdot \vec{CD} = \norm{\vec{AB}}
\times \norm{\vec{CD}} \times \cos\Bigl(\widehat{\vec{AB},\vec{CD}}\Bigr)
\end{lstlisting}
\end{code}

\noindent Matrices
\[
	\forall M \in \M_{n}, M\in\GL_{n}(\K) \iff \det\M \neq 0
\]
\begin{code}
	\begin{lstlisting}
\forall M \in \M_{n}, M\in\GL_{n}(\K) \iff \det\M \neq 0
\end{lstlisting}
\end{code}

\vspace{1em}\noindent Trigonometry, with definition of $\acos$ (\lstinline|\acos|), $\asin$ (\lstinline|\asin|) and $\atan$ (\lstinline|\atan|), as well as hyperbolic functions like $\ch$ (\lstinline|\ch|), $\sh$ (\lstinline|\sh|) and $\th$ (\lstinline|\th|),

\vspace{1em}\noindent Arithmetics (\lstinline|\div| and \lstinline|\ncgm| are also available),
\[
	\forall p\in\P,\; \forall a\in\iset{1,p},\; p \ndiv a \implies a^{p-1} \cgm{1}{p}
\]
\begin{code}
	\begin{lstlisting}
\forall p\in\P, \forall a\in\iset{1,p}, p \ndiv a \implies a^{p-1} \cgm{1}{p}
\end{lstlisting}
\end{code}

\vspace{1em}\noindent Limits and regularity classes,
\[
	\exists f \in \Func(\R) \cap \Cont(\R) \cap \Diff(\R) : f(x) \arrowlim{x\to 0} 0
\]
\begin{code}
	\begin{lstlisting}
\exists f \in \Func(\R) \cap \Cont(\R) \cap \Diff(\R) : f(x) \arrowlim{x\to 0} 0
\end{lstlisting}
\end{code}

\vspace{1em}\noindent Probabilities, with $\prob$ (\lstinline|\prob|), $\expect$ (\lstinline|\expect|) and $\variance$ (\lstinline|\variance|), comprehension sets :

\[
	[\,a,b\,[ \;= \set{x\in\R \where a \leq x < b}
\]
\begin{code}
	\begin{lstlisting}
[a,b[ = \set{x\in\R \where a \leq x < b}
\end{lstlisting}
\end{code}

\noindent ...and many more to come !

\pagebreak
\section{implementation}
\begin{code}
	\begin{lstlisting}[language=]
%% course.cls
%% Copyright 2024 RONDEAU Felix
%
% This work may be distributed and/or modified under the
% conditions of the LaTeX Project Public License, either version 1.3
% of this license or (at your option) any later version.
% The latest version of this license is in
%   https://www.latex-project.org/lppl.txt
% and version 1.3c or later is part of all distributions of LaTeX
% version 2024 or later.
%
% This work has the LPPL maintenance status `maintained'.
%
% The Current Maintainer of this work is RONDEAU Felix.
%
% This work consists of the files course.cls and course-fr.cls

%----------------------------------------------------------------------------------------
%	CLASS CONFIGURATION
%----------------------------------------------------------------------------------------

\NeedsTeXFormat{LaTeX2e}
\ProvidesClass{course}[A LaTeX class for typesetting courses.]

\LoadClass{article}


%----------------------------------------------------------------------------------------
%	PACKAGES AND OTHER DOCUMENT CONFIGURATIONS
%----------------------------------------------------------------------------------------

\usepackage{amsmath, amsfonts, mathtools, amsthm, thmtools, amssymb}
\usepackage{bm, cancel, nicefrac}
\usepackage[dvipsnames]{xcolor}
\usepackage[many]{tcolorbox}
\usetikzlibrary{decorations.pathmorphing}
\usepackage[b]{esvect}
\usepackage{siunitx}
\usepackage{titlesec} % Title formatting
\usepackage{enumitem}
\usepackage{array}
\usepackage{stmaryrd}


%----------------------------------------------------------------------------------------
%	PAGE LAYOUT
%----------------------------------------------------------------------------------------

\usepackage{geometry}

\geometry{
	paper=a4paper,
	top=1.75cm,
	bottom=1.75cm,
	left=3.5cm,
	right=2cm,
	headheight=0.75cm,
	footskip=0.5cm, % Space from the bottom margin to the baseline of the footer
	headsep=0.5cm, % Space from the top margin to the baseline of the header
	marginparwidth=.2\paperwidth,
	%showframe, % Uncomment to show how the type block is set on the page
}

% Header and Footer setup
\def\title#1{\gdef\@title{#1}}
\def\subtitle#1{\gdef\@subtitle{#1}}
\def\author#1{\gdef\@author{#1}}
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhf{}
\fancyhf[lh]{\@title}
\fancyhf[rh]{\itshape\nouppercase{\leftmark}}
\fancyhf[rf]{\thepage}


%----------------------------------------------------------------------------------------
%	FONTS
%----------------------------------------------------------------------------------------

\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}

\usepackage{raleway}
\usepackage{libertine}

% For \Re and \Im
\usepackage[scr=boondox,  % heavily sloped
	cal=cm]   % slightly sloped
{mathalpha}

% Set Linux Libertine as the default roman font
\renewcommand{\rmdefault}{LinuxLibertineT-LF}

% Set Raleway as the default sans-serif font
\renewcommand{\sfdefault}{Raleway-TLF}


%----------------------------------------------------------------------------------------
% CAPTION
%----------------------------------------------------------------------------------------

\renewcommand\maketitle{
	\thispagestyle{empty}
	\begin{center}
		\fbox{\bfseries\sffamily\LARGE\parbox{\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax}{\centering\@title}}
	\end{center}
	\vspace{-8pt}
	{\bfseries\itshape\sffamily\footnotesize \@subtitle\hfill\@author}
	\if\relax\@subtitle\relax
		\if\relax\@another\relax
			\vspace{-\baselineskip}
		\fi
	\fi
	\\
}

%----------------------------------------------------------------------------------------
% USEFUL MATHS COMMANDS
%----------------------------------------------------------------------------------------

% Common sets
\newcommand{\N}{\ensuremath{\mathbb{N}}}
\newcommand{\Z}{\ensuremath{\mathbb{Z}}}
\newcommand{\Q}{\ensuremath{\mathbb{Q}}}
\newcommand{\R}{\ensuremath{\mathbb{R}}}
\newcommand{\C}{\ensuremath{\mathbb{C}}}
\newcommand{\U}{\ensuremath{\mathbb{U}}}
\newcommand{\K}{\ensuremath{\mathbb{K}}}
\renewcommand{\P}{\ensuremath{\mathcal{P}}}
\newcommand{\M}{\ensuremath{\mathcal{M}}}
\newcommand{\GL}{\ensuremath{\mathrm{GL}}}

% Complex numbers
\let\conj\overline
\renewcommand{\Re}{\mathscr{R\!e}}
\renewcommand{\Im}{\mathscr{I\!\!m}}

% Functions set and regularity classes
\newcommand{\Func}{\mathcal{F}}
\newcommand{\Cont}{\mathcal{C}}
\newcommand{\Diff}{\mathcal{D}}

% Trigonometry
\DeclareMathOperator{\acos}{Arccos}
\DeclareMathOperator{\asin}{Arcsin}
\DeclareMathOperator{\atan}{Arctan}
\DeclareMathOperator{\ch}{ch}
\DeclareMathOperator{\sh}{sh}
\let\th\undefined
\DeclareMathOperator{\th}{th}

% Arithmetic
\renewcommand{\div}{\mid}
\newcommand{\ndiv}{\nmid}
\newcommand{\cgm}[2]{\equiv #1 \left[#2\right]} % ConGruent Modulo
\newcommand{\ncgm}[2]{\not\equiv #1 \left[#2\right]}
\newcommand{\iset}[1]{\left\llbracket #1 \right\rrbracket}

% Sets
\newcommand{\set}[1]{\{\,#1\,\}}
\newcommand{\where}{\mid}

% Vectors
\renewcommand{\vec}{\vv}
% TODO: Change font
\newcommand\norm[1]{\left\lVert#1\right\rVert}

% Limits
\newcommand{\arrowlim}[1]{\xrightarrow[#1]{}}

% Differentials
\newcommand{\dd}{\mathrm d}
%
% Probabilities
\newcommand{\prob}{\mathbb P}
\newcommand{\expect}{\mathbb E}
\newcommand{\variance}{\mathbb V}

% Other
\let\leq\leqslant
\let\geq\geqslant
\newcommand{\prop}{\mathcal P}

%----------------------------------------------------------------------------------------
% SECTION / SUBSECTION STYLE
%----------------------------------------------------------------------------------------

\newlength{\spacelength}
\settowidth{\spacelength}{\normalfont\ }

\titleformat{\section}[block]
{\bfseries\sffamily\Large}
{}
{0pt}
{\makebox[0pt][r]{Section \thesection\hspace{\marginparsep-2\spacelength}}}

\titleformat{\subsection}[block]
{\bfseries\sffamily\large}
{}
{0pt}
{\makebox[0pt][r]{Part \thesubsection\hspace{\marginparsep-2\spacelength}}}

\titleformat{\subsubsection}[block]
{\bfseries\sffamily}
{}
{0pt}
{\makebox[0pt][r]{\thesubsubsection\hspace{\marginparsep-2\spacelength}}}


%----------------------------------------------------------------------------------------
% LISTS STYLE
%----------------------------------------------------------------------------------------

% Enumerate without top margin
\newlist{enumerate-}{enumerate}{3}
\setlist[enumerate-]{nosep, itemsep=4pt, leftmargin=*, label=\arabic*.}

% Itemize without top margin
\newlist{itemize-}{itemize}{3}
\setlist[itemize-]{label=\textbullet, nosep, itemsep=4pt, leftmargin=*}


%----------------------------------------------------------------------------------------
% THEOREM STYLE
%----------------------------------------------------------------------------------------

\declaretheoremstyle[
	headfont={\bfseries\sffamily\footnotesize},
	notefont={\bfseries\sffamily\footnotesize},
	bodyfont={\normalfont},
	headpunct={\relax},%\newline,
	notebraces={}{},
	headformat={%
			\makebox[0pt][r]{\ifthenelse{\equal{\NOTE}{}}{\NAME}{\NOTE}\ \NUMBER\hspace{\marginparsep}}\hspace{-\spacelength}},
]{numbered}
\declaretheoremstyle[
	headfont={\bfseries\sffamily\footnotesize},
	notefont={\bfseries\sffamily\footnotesize},
	bodyfont={\normalfont},
	headpunct={\relax},%\newline,
	notebraces={}{},
	headformat={%
			\makebox[0pt][r]{\ifthenelse{\equal{\NOTE}{}}{\NAME}{\NOTE}\hspace{\marginparsep}}\hspace{-\spacelength}},
]{unnumbered}
\declaretheoremstyle[
	headfont={\footnotesize\itshape},
	notefont={\normalfont},
	bodyfont={\normalfont},
	headpunct={\relax},
	headformat={%
			\makebox[0pt][r]{\ifthenelse{\equal{\NOTE}{}}{\NAME}{\NOTE}\hspace{\marginparsep}}\hspace{-\spacelength}},
]{claim}

\theoremstyle{unnumbered}
\newtheorem{blank}{}


% Theorem
\declaretheorem[name=Theorem, style=numbered]{theorem}
\tcolorboxenvironment{theorem}{
	boxrule=0pt,
	boxsep=0pt,
	enhanced jigsaw,
	opacityback=0,
	colframe=black,borderline={1pt}{0mm}{black},
	sharp corners,
	before skip=8pt,
	after skip=8pt,
	left=5pt,
	right=5pt,
	breakable,
}

\newenvironment{theorems}
{\begin{theorem}[Theorems]}{\end{theorem}}

\newenvironment{theoremlist}
{\begin{theorems}\begin{enumerate-}}{\end{enumerate-}\end{theorems}}


% Proof and building
\let\proof\relax
\declaretheorem[
	name=Proof,
	style=unnumbered,
	qed=\qedsymbol]{proof}

\tcolorboxenvironment{proof}{
	boxrule=0pt,
	boxsep=0pt,
	blanker,
	borderline west={0.5pt}{0pt}{black!60!white},
	before skip=8pt,
	after skip=8pt,
	left=5pt,
	right=5pt,
	breakable,
}

\newenvironment{building}
{\begin{proof}[Building]}{\end{proof}}

\newenvironment{proofs}
{\begin{proof}[Proofs]}{\end{proof}}

\newenvironment{prooflist}
{\begin{proofs}\begin{enumerate-}}{\end{enumerate-}\end{proofs}}


% Definition
\theoremstyle{numbered}
\newtheorem{definition}{Definition}

\newenvironment{definitions}
{\begin{definition}[Definitions]}{\end{definition}}

\newenvironment{deflist}
{\begin{definitions}\begin{itemize}[nosep, itemsep=4pt, leftmargin=*]}{\end{itemize}\end{definitions}}

\tcolorboxenvironment{definition}{
	boxrule=0pt,
	boxsep=0pt,
	enhanced jigsaw,
	opacityback=0,
	colframe=black,borderline={1pt}{0mm}{black,dashed},
	sharp corners,
	before skip=8pt,
	after skip=8pt,
	left=5pt,
	right=5pt,
	breakable,
}

% Properties
\theoremstyle{numbered}
\newtheorem{property}{Property}

\newenvironment{properties}
{\begin{property}[Properties]}{\end{property}}

\newenvironment{proplist}
{\begin{property}\begin{itemize}[nosep, itemsep=4pt, leftmargin=*]}{\end{itemize}\end{property}}

\tcolorboxenvironment{properties}{
	boxrule=0pt,
	boxsep=0pt,
	enhanced jigsaw,
	opacityback=0,
	colframe=black,borderline={1pt}{0mm}{black},
	sharp corners,
	before skip=8pt,
	after skip=8pt,
	left=5pt,
	right=5pt,
	breakable,
}

% Lemma
\theoremstyle{numbered}
\newtheorem{lemma}{Lemma}
\tcolorboxenvironment{lemma}{
	boxrule=0pt,
	boxsep=0pt,
	blanker,
	borderline west={0.8pt}{0pt}{decoration={snake,aspect=0.3,segment length=3mm,amplitude=1pt},decorate},
	before skip=8pt,
	after skip=8pt,
	sharp corners,
	left=5pt,
	right=5pt,
	breakable,
}

% Corollary
\theoremstyle{numbered}
\newtheorem{corollary}{Corollary}
\tcolorboxenvironment{corollary}{
	boxrule=0pt,
	boxsep=0pt,
	blanker,
	borderline west={0.8pt}{0pt}{decoration={zigzag,amplitude=1pt,pre=moveto,segment length=5pt,angle=42},decorate},
	before skip=8pt,
	after skip=8pt,
	sharp corners,
	left=5pt,
	right=5pt,
	breakable,
}

% Remarks
\theoremstyle{unnumbered}
\newtheorem{remark}{Remark}

\newenvironment{remarks}
{\begin{remark}[Remarks]}{\end{remark}}

\newenvironment{remarklist}
{\begin{remarks}\begin{itemize}[nosep, itemsep=4pt, leftmargin=*]}{\end{itemize}\end{remarks}}

\tcolorboxenvironment{remark}{
	boxrule=0pt,
	boxsep=0pt,
	blanker,
	borderline west={1pt}{0pt}{black,dotted},
	before skip=8pt,
	after skip=8pt,
	sharp corners,
	left=5pt,
	right=5pt,
	breakable,
}

% Example
\theoremstyle{numbered}
\newtheorem{example}{Example}

\newenvironment{examples}
{\begin{example}[Examples]}{\end{example}}

\newenvironment{exlist}
{\begin{examples}\begin{itemize}[nosep, itemsep=4pt, leftmargin=*]}{\end{itemize}\end{examples}}

\tcolorboxenvironment{example}{
	boxrule=0pt,
	boxsep=0pt,
	blanker,
	borderline west={1pt}{0pt}{black},
	sharp corners,
	before skip=8pt,
	after skip=8pt,
	left=5pt,
	right=5pt,
	breakable,
}
\end{lstlisting}
\end{code}

\end{document}
